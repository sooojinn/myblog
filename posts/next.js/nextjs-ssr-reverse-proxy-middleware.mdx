...
access token이 쿠키에 저장됐으니 api 요청 시 쿠키에서 access token 값을 빼와서 Authorization 헤더에 추가해 보내면 된다. 문제는 서버 컴포넌트에서는 쿠키에 문제 없이 접근 가능하나 클라이언트 컴포넌트에서는 접근하지 못한다는 것이다. (보안을 위해 HttpOnly 쿠키로 설정했기 때문에) 미들웨어에서 쿠키에 접근이 가능하다고 해서 api 요청 시 미들웨어에서 해당 로직을 처리하기로 했다.
