...
가상 리스트란 나열된 리스트의 아이템 중 화면에 실제로 보이는 아이템만 렌더링하는 기술이다. 이 기능을 도와주는 라이브러리가 여러 개 있는데 그중 가장 가볍고, [공식 문서](https://tanstack.com/virtual/latest)가 친절하고, Tanstack에서 만든 `react-virtual`이라는 라이브러리를 사용했다. 공식 문서 외에는 자료가 많이 없어 조금 고민했지만 일단 해보기로 했다.
