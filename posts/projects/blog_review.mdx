...
자바스크립트보다 타입스크립트를 사용하는 것이 권장된다. 하지만 이 프로젝트를 시작했을 땐 타입스크립트를 배우지 않은 상태였기 때문에 자바스크립트를 선택했다. 추후에 타입스크립트로 업데이트할 생각이다!
...
![Untitled](/posts/projects/blog_review_1.png)
...
- 타입스크립트와 tailwind css를 사용하지 못한 것이 아쉽다. Next.js도 처음 써보는 거라 나머지는 내게 익숙한 것으로 선택했다. 가능하면 추후에 업데이트 할 생각이다.
