...
export async function getPostMetaData(category, slug) {
  const parsedData = parseMdxFile(category, slug);
  return parsedData.data;
}

export async function getPostMainText(category, slug) {
  const parsedData = parseMdxFile(category, slug);
  return parsedData.content;
}

export function getPostPaths(category) {
  const folder = category || "**";
  const postPaths = sync(`${postsDirectory}/${folder}/*.mdx`);
  return postPaths;
}

export async function getSortedPostList(category) {
...
export function changeCategoryName(category) {
...
export async function getRenderedCategoryList() {
